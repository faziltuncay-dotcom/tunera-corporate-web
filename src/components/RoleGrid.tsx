...
type Props = {
  items: Item[];
  /**
   * Layout columns at lg+. Defaults to 2 — keeps role/principle cards
   * editorial rather than dashboard-grid dense.
   */
  cols?: 2 | 4;
};

export function RoleGrid({ items, cols = 2 }: Props) {
  const grid =
    cols === 4
  ...
  return (
    <ul role="list" className={grid}>
      {items.map((item, i) => (
        <li
          key={item.title}
          className="rounded-md border border-tunera-stone/60 bg-white p-6 transition-colors hover:border-tunera-ink/20 sm:p-7"
        >
